...
const AUTOCOMPLETE_COMMANDS = [
  'summon', 'dismiss', 'walk', 'scan', 'collect', 'absorb',
  'broadcast', 'check coherence', 'dispatch', 'look',
  'Builder-7', 'Scout-3', 'Scholar-1', 'Critic-2',
];

export default function MudCommandParser() {
  const [input, setInput] = useState('');
  const [logs, setLogs] = useState<LogEntry[]>([
    { id: 'l0', text: 'Welcome to Agentic Scratch. Type a command or speak to your agent.', type: 'system', timestamp: 0 },
  ]);
    ...
    <div className="w-full bg-[#0A0A0F] border-t border-[#252536] flex flex-col">
      ...
      <div className="relative px-4 py-2">
        <div className="flex items-center gap-3">
          <span className="text-[#E8B820] font-terminal text-lg select-none">&gt;</span>
          <input
            ref={inputRef}
            type="text"
            value={input}
          ...
          <motion.button
            ...
            className="p-1.5 rounded bg-[#E8B820]/20 hover:bg-[#E8B820]/30 transition-colors"
          >
            <Send className="w-4 h-4 text-[#E8B820]" />
          </motion.button>
        </div>
              ...
              {matches.map((match, i) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-1.5 text-sm font-mono transition-colors ${
                    i === selectedIdx ? 'bg-[#E8B820]/15 text-[#E8B820]' : 'text-[#8A8AA8] hover:bg-[#252536]'
                  }`}
                >
                  {match}
                </button>
